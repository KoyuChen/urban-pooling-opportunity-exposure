...
|---|---|---|---|---|
| Observation operator and target population | The public row reveals realized co-presence at the node level but does not publish partner, provider, vehicle, or group identifiers | Field-to-claim map, live cross-tabs, and minimal counterexamples | High-level current documentation complete | Run closure, inconsistent fields, implementation/null causes, and production boundary audit remain unverified |
...
| Suppression-coupled geography | Current-linked City documentation describes an at-most-two threshold with paired endpoint coarsening; the clarification gives separate pickup/start and dropoff/end cells | Joint endpoint-assignment/matching formulation, explicit-DNF LOW/HIGH release compiler, and exact tests | Documented direction compiled; City-specific adapter open | Implementation partitions, blank causes, late-row recomputation, DST, and tract vintage |
...
| Validation design | The generator embeds outcome-separating and SES-geographic structure; the current primary ablation was selected after inspecting the original diagnostic | Reproduction, leakage audit, UCI real-topology boundary, and FEBRL4 method-fit test | Partial external repair | Full UCI scan, realistic candidate recall, and natural calibration markets |
| Novelty and venue positioning | Linkage ranges, budgeted matching, conformal matching, bounded-width CSP, knowledge compilation, and Chicago pooling all have predecessors | Primary-source matrix plus DNF/OBDD counterexamples | Conditional integrated increment survives: release-coupled frontier, weak-hardness boundary, witnesses, and outward certificate | Run-closed production scaling and full-corpus external evidence remain submission gates |
...
| External relation-truth boundary | Real linkage truth need not satisfy matching topology; forcing it would fabricate the model fit | UCI block-1 positive-component scan and truth-conditioned dyad frontier; FEBRL4 complete-matching audit | Pinned official cache/package, isolated IDs/truth, disclosed truth-conditioned markets | Record-linkage benchmark practice | Partial pass | All-ten-block UCI scan and natural market sampling remain |
...
blockers: the current declared-input suites have 96 deterministic tests, including 80
...
reference implementation on declared worlds only; the 23-case profile and
external audits retain their narrower evidence boundaries.
