...
informativeness. The public NYC diagnostics show that a richer model can add
fixed-support worlds without changing the extrema of particular queries.
These are distinct scientific conclusions, not interchangeable notions of
success.
...
Our frozen diagnostics report null endpoints, including four views selected
outcome-blind for non-clique geometry. Ordered events add worlds in one cell,
so this is not model equality; thirteen census cells remain transport-unresolved.
Chicago and ATR provide lower-complexity audits, not evidence for the value of
higher-cardinality ordered events.
