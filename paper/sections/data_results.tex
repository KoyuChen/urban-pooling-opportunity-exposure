...
but includes computationally unresolved pairs.}
...
same eligibility and 60-second endpoint budgets. All have terminal records:
90 are complete and six are scientifically ineligible. Of 10,762 pairs with
complete public query values, 10,728 close numerically and 34 retain valid
incumbent/bound information without a numerical endpoint certificate. Thus a
completed window does not mean every query is certified. Across the two
calendar-disjoint designs, 113 of 120 declared windows are complete and seven
are ineligible; neither design is a probability sample.

\paragraph{Stable support sensitivity can coexist with weak identification.}
Across the 90 completed follow-up windows, median duration-gap width rises
from 40.748 minutes per core trip at the 0-km measured-radius support to 44.234
under temporal-only support; the within-window paired median increase is 2.599
minutes. Under the separate generic omitted-incidence model, 86 of 87
numerically comparable duration chains are within 1\% of their own
full-support width by budget $\Gamma=8$. The effect of omitted candidates is
therefore concentrated but nonzero. These are paired descriptive summaries
on declared candidate graphs, not independent samples, operational partner
coverage, or citywide prevalence.
...
numerically and 25 remain numerically unresolved;
...
A retrospective $4+12$ audit compares models at every common positive $q$;
none of the 24 endpoint-pair comparisons changes. All 16 intervals share 14 seconds, so every
capacity-feasible ordered event is a clique, the collapse case of
Proposition~\ref{prop:modelcontainment}. Ordered events reach more buffers than
pairs at $C=3,4$, but that support difference changes the estimand. The fixed
cohort therefore remains an informative endpoint null.

A subsequent 24-cell geometry census leaves eight geometry-complete, three scientifically ineligible, and thirteen
transport-unresolved cells. Every completed full view contains a core-touching
non-clique column, which does not by itself establish a complete feasible world
or a fixed-$q$ outcome effect.

Before attaching query values, a follow-up retains exactly the four completed
$4+12$ views with a core-touching induced path; all reproduce their frozen
projection and geometry hashes. Of 192 comparisons over two queries, three
capacities, four supports, and two restrictions, 96 are common-support eligible
and certified. Ordered events admit more buffer worlds than cliques in 1/24
common-feasible cells (and pairs in 3/24), yet endpoints change in 0/48
ordered--clique and 0/48 ordered--pair comparisons. This is a conditional null,
not model equivalence. The remaining thirteen transport-unresolved census cells
stay in the denominator; this is neither a holdout nor evidence of true-event
recovery or population prevalence.

An exact attainment audit explains the null without changing windows, supports,
or queries. In all 192 lower/upper endpoint--restriction checks, a restricted
family attains the ordered value using the same selected-buffer subset. Across
the 48 restriction world cells, 36 add only event partitions, four add projected
buffer subsets, and eight coincide at both levels. The added subsets are
strictly interior for both declared additive queries: none ties an audited
endpoint. Hence partition structure can expand without changing projected
support, and projected support can expand without changing a particular
additive frontier.
...
On a snapshot-consistent reconstruction of the four slowest cells, an exact
geometry cache preserves every certificate and branch path while cutting
actual pricing-LP calls by 29.6\%. The old artifacts lack fixed-input hashes,
so this is not a byte-identical replay and timing remains descriptive.
This establishes certified support optimization on one deterministic cohort,
but does not close the 25 numerically unresolved outcome pairs, provide a
city-scale runtime guarantee, or recover true memberships. The evidence
supports conditional computation and substantial relation ambiguity; the
public non-clique diagnostic detects extra worlds but not an endpoint advantage
for the two declared queries.
