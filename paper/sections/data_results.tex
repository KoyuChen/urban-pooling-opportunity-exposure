...
Before attaching query values, a follow-up retains four completed $4+12$ views
with a core-touching induced path; all replay their frozen hashes. All 96
common-support comparisons certify. Ordered events add buffer worlds versus
cliques in 1/24 common-$q$ cells and pairs in 3/24, but 0/48 endpoints move for
either restriction. The census denominator remains 8 complete, 3 ineligible,
and 13 transport-unresolved; this conditional null is neither model equivalence
nor evidence of true-event recovery or prevalence.

Exact replay explains the null without changing queries: in all 192 lower/upper
checks, a restricted family attains the ordered value on the same buffer subset.
Of 48 restriction cells, 36 add only event partitions, four add projected
subsets, and eight add neither; no added subset ties a declared-query endpoint.
Thus a family can expand before its projected additive frontier does. This is
the single-query case of Proposition~\ref{prop:projectionfrontier}, not a claim
that all additive frontiers coincide.
