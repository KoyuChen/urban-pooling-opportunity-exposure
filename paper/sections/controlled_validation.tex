...
\]
This is a positive control: when both the true world and a point world belong
to the optimized set and disagree at a threshold, the frontier must cross that
threshold. The result checks implementation consistency; it does not establish
unconditional coverage, optimal abstention, or a new general certification
principle.
...
relation universe. Worse, truncation can create false precision. At six retained
buffers, the truncated frontier certifies 63.4--64.0\% of threshold cells, but
6.9--7.7\% of those certificates disagree with known truth. With all eight
buffers, only 32.1--35.6\% of threshold cells are certified, but no certificate
is false. A higher certification rate under a smaller candidate universe is
therefore not evidence of better identification.

The correct workflow reports candidate-support sensitivity as a separate
identification layer. Pair matching and connected components are
