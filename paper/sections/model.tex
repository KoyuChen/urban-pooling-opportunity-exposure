...
For nonempty $\F$, bounds $L\le\underline H$ and $U\ge\overline H$ certify
positive if $L\ge\eta$ and negative if $U<\eta$. Feasible values
$h^-<\eta\le h^+$ certify ambiguity without optimal endpoints. Exact endpoints
make the three cases exhaustive. A directional certificate agrees with any
true world in $\F$; without coverage, no truth guarantee follows.
\end{theorem}
...
Adding optional buffers without changing old rules expands the reachable
values at fixed $q$, so its frontier weakly expands
(Proposition~\ref{prop:candidatesupport}). This does not compare endogenous
maximum counts or imply truth coverage.
...
Proposition~\ref{prop:projectionfrontier} sharpens the converse: one additive
frontier stays equal exactly when the restricted projection hits both ordered
exposed faces, while equality for every selected-row additive outcome is
equivalent to equality of the two 0--1 projections, not of event partitions.
