...
timestamp completion. A fixed-$q$ separation changes a row-additive aggregate
under the event model; a collapse condition explains when it cannot differ
from cliques. An overlap-margin equivalence connects the continuous MILP to
the same strict-overlap object. Pairwise compatibility and outer time
envelopes cannot substitute for joint feasibility.
...
identify an aggregate class preserving its pricing structure, and retain
explicit bounds when search is incomplete. No global hardness result is claimed.
...
members but only 31--33\% of complete true worlds; it also produces false
certificates in 6.9--7.7\% of apparently certified decisions. In the annotated dyad
...
pairs, with 686 pairs excluded for missing public values. Its calendar-disjoint
96-window follow-up returns 90 complete and six ineligible windows; 10,728 of
10,762 data-complete endpoint pairs close numerically and 34 remain unresolved.
NYC has 125 witness-certified ambiguous decisions
among 126 outcome--capacity cells, while a separate 18-cell support-maximization
lattice closes through 16 cores and 48 buffers. A fixed clique input changes no
endpoint in 24 model comparisons. Four views selected outcome-blind for
non-clique geometry add ordered worlds in 1/24 common-support cells relative to
cliques, but change 0/48 certified endpoints. Thus public-data evidence supports
conditional ambiguity and bounded-scale computation, not an empirical outcome
advantage. This distinction bounds our claims.
