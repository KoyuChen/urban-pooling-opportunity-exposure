...
\begin{proposition}[Projection-to-frontier equivalence]
\label{prop:projectionfrontier}
Let $\mathcal W_R\subseteq\mathcal W_O$ be nonempty fixed-$q$ world families,
and let $\mathcal Z_R\subseteq\mathcal Z_O\subseteq\{0,1\}^{B_0}$ be their
selected-buffer projections. For a selected-row additive outcome
$H_w(W)=a+\rho w^\top z(W)$ with fixed $a$ and $\rho>0$, define the exposed
minimum and maximum faces of $P_O=\operatorname{conv}(\mathcal Z_O)$ under
$w$:
\[
 E_O^-(w)=\arg\min_{z\in P_O}w^\top z,
 \qquad E_O^+(w)=\arg\max_{z\in P_O}w^\top z.
\]
The two families have the same lower and upper endpoints for this $w$ if
and only if $\mathcal Z_R$ intersects both faces. They have the same endpoints
for every $w\in\mathbb R^{B_0}$ if and only if
$\mathcal Z_R=\mathcal Z_O$ (equivalently, their convex hulls agree).
\end{proposition}
\begin{proof}
Because $H_w$ factors through $z(W)$, its extrema over either world family are
the extrema of the affine functional over the corresponding finite projection.
Nestedness implies equality of the lower endpoint exactly when some point of
$\mathcal Z_R$ attains the ordered minimum, and likewise for the maximum; this
is precisely intersection with both exposed faces.

Equal projections plainly give equal endpoints for every $w$. Conversely,
take $z^*\in\mathcal Z_O\setminus\mathcal Z_R$ and set
$w_j=1$ when $z^*_j=1$ and $w_j=-1$ otherwise. For every other binary $z$,
$w^\top z^*-w^\top z$ is their positive Hamming distance. Thus $z^*$ is the
unique maximizer over $\mathcal Z_O$, while the restricted maximum is strictly
smaller, a contradiction. Equality of all linear extrema is also equivalent
to equality of the convex hulls; the constructive argument shows that finite
0--1 projections themselves must then agree.
\end{proof}

\paragraph{Minimal separation of projection and one frontier.}
At $q=1$, let $\mathcal Z_R=\{100,001\}$ and
$\mathcal Z_O=\{100,010,001\}$. For $w=(0,1,2)$ both frontiers are $[0,2]$,
but the ordered-only point has value one and is strictly interior. Three
buffers are minimal for such a strict-interior omission under distinct
weights: with two buffers, a strict nonempty fixed-$q$ inclusion leaves a
singleton restriction, so adding the other point changes an endpoint.
Separately, with four mutually overlapping exact rows at $C=2,q=2$, the partitions
$\{\{c_1,b_1\},\{c_2,b_2\}\}$ and
$\{\{c_1,b_2\},\{c_2,b_1\}\}$ have the same projection. Adding the second
can enlarge a partition family without changing any selected-row additive
frontier. Partition-dependent outcomes such as event count are outside the
proposition. These witnesses explain possible mechanisms; they are not new
public-data queries.
...
\paragraph{Proof of Theorem~\ref{thm:decisioncertification}.}
For every $F\in\F$, $H(F)\ge\underline H\ge L$ and
$H(F)\le\overline H\le U$. The first two certificates follow immediately.
The two feasible witnesses produce opposite threshold decisions, proving
ambiguity without an optimality claim. Attainment of the finite-partition
extrema makes $\underline H\ge\eta$, $\overline H<\eta$, and
$\underline H<\eta\le\overline H$ exhaustive when endpoints are exact. If a
true world is in $\F$, the universal statements apply to it. If it is absent,
the theorem makes no statement about its decision.

\begin{proposition}[Candidate-support nesting]
\label{prop:candidatesupport}
Hold cores, capacity, a fixed feasible $q$, event rules, and old-row supports
fixed. Adding optional buffers preserves every old reachable value whenever
$H$ agrees on old-row worlds; therefore the lower endpoint cannot increase
and the upper endpoint cannot decrease.
\end{proposition}
\begin{proof}
Take an old world and choose any allowed completion for every newly added
buffer, leaving each new buffer unused. Optionality and the unchanged event
rules preserve every old event, core assignment, and selected-buffer count.
The compatibility assumption preserves its value of $H$. Thus the old
reachable-value set is contained in the new one, which gives the endpoint
inequalities. Endogenous support counts compare different feasible slices;
changing old rules can remove old worlds; and truth coverage is a separate
assumption, so none is implied by this inclusion.
\end{proof}
...
\section{A Tractable Global Boundary and a Fixed-Support Gap}
\begin{proposition}[Low-depth closure]
\label{prop:lowdepth}
Let exact intervals have full-family maximum occupancy at most $C$, and let
$\mathcal K$ be the connected components of their positive-overlap graph. If
some $K\in\mathcal K$ contains a core and $|K|=1$, no world exists. Otherwise,
\[
 M=\sum_{K\in\mathcal K:K\cap C_0\ne\varnothing}|K\cap B_0|.
\]
Moreover, if every component of the core-induced overlap graph has at least
two vertices and every buffer overlaps a core, then
$Q=\{0,1,\ldots,|B_0|\}$.
\end{proposition}

\begin{proof}
Capacity is automatic for every subset. A full component containing a core is
itself one connected event unless it is a singleton; taking all such
components covers all cores and selects every buffer in them. A buffer in a
component without a core cannot belong to any feasible event, proving the
formula and singleton obstruction.

For the final claim, begin with one event for each core-induced component;
each already has two rows. For any chosen subset of buffers, assign each
chosen buffer to one such event containing an overlapping core. The resulting
events remain connected, disjoint, and within capacity. This constructs a
world for every chosen subset and hence every stated cardinality.
\end{proof}

The extra condition is necessary for the no-gap conclusion. At $C=2$, take
cores $c_0=[0,7)$ and $c_1=[3,5)$ and buffers $b_0=[2,4)$ and
$b_1=[4,6)$. At $q=0$, $\{c_0,c_1\}$ is an event; at $q=2$, use
$\{c_0,b_0\}$ and $\{c_1,b_1\}$. At $q=1$, the chosen buffer cannot be
joined to both cores without triple occupancy, while separating the cores
leaves one without a companion. Thus $Q=\{0,2\}$. In particular, deciding
whether support is at least one does not decide feasibility at exactly one.
...
Across these 18 cells, the solver performs 2,389,799 fixed-span oracle LP
solves but processes only 59 branch nodes (at most 11 in one cell).  The four
slowest cells consume 88.0\% of runtime; on $\log(1+x)$ scales, runtime
correlates 0.986 with oracle-LP volume and 0.544 with processed nodes.  This
descriptive profile localizes repeated pricing work as the main scaling signal
on this lattice; it is not a causal ablation or runtime-complexity claim.
An objective-independent cache subsequently memoized only boxes rejected by
an exact integer-arithmetic geometry precheck, never a numerical LP status.
In a paired constructed audit (five cells, two repetitions), enabling that
cache preserved every integer certificate and branch path while reducing
actual fixed-span LP calls from 9,900 to 7,770 (21.5\%).  A second paired audit
reconstructed the four dominant public cells from the matching dataset
fingerprint, provider, time window, and source counts.  Every cache-off run
reproduced the historical integer certificate and branch path; cache-on
preserved them while reducing actual LP calls from 1,880,627 to 1,324,841
(29.6\%), with a runner-specific time win in all four pairs.  The old artifacts
retained neither rows nor an input hash, so this is a snapshot-consistent
deterministic reconstruction, not a byte-identical replay or city-scale claim.
\begin{table*}[t]
\caption{All 18 predeclared NYC integer scale cells. A bracket is the valid
[integer incumbent, global upper bound] interval at termination. ``Certified''
means bound closure plus witness replay under recorded tolerances, not exact arithmetic.}
